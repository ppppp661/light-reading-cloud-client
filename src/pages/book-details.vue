  ...
  <v-app cols="12" sm="12" align="center" offset-sm="3">
    ...
    <v-content style="width: 900px; text-align: left">
      <v-container fluid>
        <v-row align="center">
          <v-col cols="6" sm="6">
            <v-img height="252" width="180" :src="book.imgUrl"></v-img>
          </v-col>
          <v-col cols="6" sm="6" no-gutters>
            ...
            <v-card-text>
              ...
              <div class="my-4 subtitle-1">作者：{{ book.authorName }}</div>
      ...
      <div style="height: 30px">
        ...
        <v-dialog v-model="dialog" persistent max-width="290">
          <v-card>
            ...
            <v-card-text
              >您还没有登录，还不能加入书架哦，是否跳转到登录页面？</v-card-text
            >
            <v-card-actions>
              <v-spacer></v-spacer>
              <v-btn color="white darken-1" @click="dialog = false">算了</v-btn>
              <v-btn color="blue darken-1" @click="gotoLogin()"
                >嗯，去登录</v-btn
              >
            </v-card-actions>
          </v-card>
...
export default {
  ...
  methods: {
    ...
    addBookshelf() {
      ...
      this.postRequest("/account/bookshelf/sync-book", dataForm, headers).then(
        (resp) => {
    ...
    likeSeeClick() {
      ...
      this.postRequest("/account/like-see/click", dataForm, headers).then(
        (resp) => {
          if (resp && resp.code == 200) {
