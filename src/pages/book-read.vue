    ...
    >
      ...
      <v-menu left bottom>
        ...
        <v-list>
          ...
          <v-list-item>
            <v-list-item-title @click="$router.push('/book-details/' + bookId)"
              >图书详情</v-list-item-title
            >
          </v-list-item>
          <v-list-item>
        ...
        <v-row>
          <v-col cols="12" sm="12">
            <div
              ...
              style="font-size: 18px; line-height: 34px; white-space: pre-line"
            >
              {{ current.content }}
            </div>
          </v-col>
...
export default {
  ...
  methods: {
    ...
    getChapterInfo: function (bookId, chapterId) {
      console.log();
      this.getRequest("/book/chapter/readChapter", {
        bookId: bookId,
        chapterId: chapterId,
      }).then((resp) => {
        if (resp.code == 200) {
    ...
    // 获取章节目录
    getCatalog() {
      this.getRequest("/book/chapter/getChapterList", {
        bookId: this.bookId,
      }).then((resp) => {
        if (resp.code == 200) {
