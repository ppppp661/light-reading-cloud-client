    ...
    <v-row v-for="item in items" :key="item.id">
      ...
      <div v-if="item.itemType == 1" class="item">
        ...
        <v-container v-if="item.booklist.style == 1">
          <booklist-two-by-three
            :booklist="item.booklist"
          ></booklist-two-by-three>
        </v-container>
        <!-- 2.竖版(1*3) -->
        <v-container v-else-if="item.booklist.style == 2">
          <booklist-one-by-three
            :booklist="item.booklist"
          ></booklist-one-by-three>
        </v-container>
        <!-- 3.1+4本 -->
        <v-container
          v-else-if="item.booklist.style == 3 && item.booklist.books.length > 1"
        >
          <booklist-one-plus-four
            :booklist="item.booklist"
          ></booklist-one-plus-four>
        </v-container>
        <!-- 4.推荐1本 -->
        <v-container
          v-else-if="item.booklist.style == 4 && item.booklist.books.length > 0"
        >
          <booklist-one :booklist="item.booklist"></booklist-one>
        </v-container>
        <!-- 4.竖版(1*5) -->
        <v-container v-else-if="item.booklist.style == 5">
          <booklist-one-by-five
            :booklist="item.booklist"
          ></booklist-one-by-five>
        </v-container>
      </div>
...
import booklistOneByFive from "@/components/booklist-one-by-five";
